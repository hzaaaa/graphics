...
static Eigen::Vector2f interpolate1(float alpha, float beta, float gamma, const Eigen::Vector2f &vert1, const Eigen::Vector2f &vert2, const Eigen::Vector2f &vert3, float weight)
{
...
void rst::rasterizer::rasterize_triangle(const Triangle &t, const std::array<Eigen::Vector3f, 3> &view_pos)
{
    ...
    for (int x1 = min_x; x1 <= max_x; x1++)
    {
        for (int y1 = min_y; y1 <= max_y; y1++)
        {
            ...
            bool flag = insideTriangle(x1+ 0.5f, y1 + 0.5f, t.v);
            ...
            if (flag)
            {
                ...
                mindpth = std::min(mindpth, zp);

                // 如果x,y所在点的深度小于z-buffer的深度，
                if (depth_buf[get_index(x1, y1)] > mindpth)
                {
                    ...
                    //
                    Eigen::Vector2f interpolated_texcoords = interpolate1(alpha, beta, gamma, t.tex_coords[0], t.tex_coords[1], t.tex_coords[2], 1.0f);
